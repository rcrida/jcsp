...
- **Consistency preprocessing**: AC3 arc consistency, node consistency, AllDiff GAC (Régin 1994), SumBoundConstraint/SumVariableConstraint and LinearBoundConstraint/LinearVariableConstraint bounds propagation, LinearBooleanBoundConstraint/LinearBooleanVariableConstraint boolean-indicator weighted-sum propagation, CountConstraint and AmongConstraint value-set propagation, InverseConstraint arc consistency, AtLeastN/AtMostN/AtMostOne/ExactlyOne boolean forcing, CumulativeConstraint timetabling propagation, DisjunctiveConstraint edge-finding (Carlier & Pinson 1989; Vilim 2007) — a strictly stronger unary-resource propagator than timetabling alone, GlobalCardinalityConstraint/GlobalCardinalityVariableConstraint GAC (Régin 1996, flow-based), NValueConstraint bounds-consistency propagation, BinPackingConstraint per-bin load propagation, LexConstraint bounds propagation, MaxConstraint and MinConstraint bounds propagation, ProductConstraint/ProductVariableConstraint bounds propagation, DivisionConstraint bounds propagation, NaryElementConstraint domain filtering, NaryTuplesConstraint/NaryStarredTuplesConstraint/NaryConflictTuplesConstraint table GAC (positive, wildcard, and negative tables — none needs an exponential complement or expansion), CircuitConstraint Hamiltonian propagation, DiffnConstraint/DiffnVariableConstraint compulsory-part propagation, RegularConstraint forward-backward DP propagation, IncreasingConstraint/DecreasingConstraint bounds consistency, and ReifiedConstraint/ImplicationConstraint delegated propagation — all run in a combined fixpoint loop so each propagator benefits from the others' reductions
- **Flexible constraint types**: unary, binary (equals, not-equals, offset, comparator, logic, element over fixed array, absolute-difference, division, predicate, tuples, subset, disjoint, intersection-cardinality), and n-ary (AllDiff, AtMostOne, AtLeastN, AtMostN, ExactlyOne, Sum, Product, Linear, Count, Among, Inverse, GlobalCardinality, NValue, Cumulative, Disjunctive, BinPacking, Max, Min, Element over variables, Tuples (positive, wildcard/starred, and negative/conflict), ValueDisjunction (`OR(v_i == target_i)`), ValueConjunction (`AND(v_i <op> target_i)`), Increasing, Decreasing, Lex, predicate, Circuit, Diffn, Regular)
...
- **XCSP3 instance parsing**: `Xcsp3Parser.parse(Path)` reads an [XCSP3](https://xcsp.org) instance file directly into a `ConstraintSatisfactionProblem` (plus an optional objective for COP instances), covering integer and symbolic (string-valued) variable domains, `intension` (including `iff`/`xor` and, for a two-operand `iff` of otherwise-recognizable relations, real reified propagation rather than an opaque predicate), `extension`, `allDifferent` (including the multi-list "distinct vectors" form), `allEqual`, `sum`, `count`/`among`, `minimum`/`maximum`, `element`, `ordered`, `lex`, `cumulative`, `noOverlap` (1D via real edge-finding, 2D via compulsory-part propagation), `circuit`, `binPacking`, `regular`, `mdd`, `channel`, `instantiation`, `clause` (SAT), and single-variable/sum-type/maximum-type objectives
