...
- **Optimization**: branch-and-bound search via `createSolver(csp, objective)` — returns a `BoundSolver` whose `getSolution()` finds the global optimum and `getSolutions()` streams improving assignments. Solution-guided: each improving solution's values are tried first when branching thereafter, steering search back toward the best region found rather than re-deriving it value by value. For an integral `LinearObjective`, each new incumbent is also applied as a propagated *constraint* (`sum(c*v) <= incumbent - 1`), not merely as a pruning test — so every other propagator can compound with it, which is what lets optimality actually be proved rather than just reached
- **Consistency preprocessing**: AC3 arc consistency, node consistency, AllDiff GAC (Régin 1994), SumBoundConstraint/SumVariableConstraint and LinearBoundConstraint/LinearVariableConstraint bounds propagation — with real value-level GAC for the `EQ` case over discrete domains via a forward-backward subset-sum DP, closing a gap plain bounds consistency can't (e.g. `x1∈{0,3}, x2∈{0,5}, x1+x2==4` looks achievable by range alone but no real combination reaches it) — LinearBooleanBoundConstraint/LinearBooleanVariableConstraint boolean-indicator weighted-sum propagation, CountConstraint and AmongConstraint value-set propagation, InverseConstraint arc consistency, AtLeastN/AtMostN/AtMostOne/ExactlyOne boolean forcing, NotAllEqualConstraint exact GAC, CumulativeConstraint/CumulativeVariableConstraint timetabling (plus energy-overload detection) propagation, DisjunctiveConstraint edge-finding (Carlier & Pinson 1989; Vilim 2007) — a strictly stronger unary-resource propagator than timetabling alone, GlobalCardinalityConstraint/GlobalCardinalityVariableConstraint GAC (Régin 1996, flow-based), NValueConstraint bounds-consistency propagation, BinPackingConstraint per-bin load propagation, LexConstraint bounds propagation, MaxConstraint/MaxVariableConstraint and MinConstraint/MinVariableConstraint bounds propagation — with full value-level GAC for the variable-target `EQ` case — ProductConstraint/ProductVariableConstraint bounds propagation, DivisionConstraint bounds propagation, NaryElementConstraint domain filtering, NaryTuplesConstraint/NaryStarredTuplesConstraint/NaryConflictTuplesConstraint table GAC (positive, wildcard, and negative tables — none needs an exponential complement or expansion, and positive tables use a bitset-indexed inverted support index for large instances), CircuitConstraint Hamiltonian propagation, DiffnConstraint/DiffnVariableConstraint compulsory-part propagation, RegularConstraint forward-backward DP propagation, IncreasingConstraint/DecreasingConstraint bounds consistency, and ReifiedConstraint/ImplicationConstraint delegated propagation — all run in a combined fixpoint loop so each propagator benefits from the others' reductions
- **Flexible constraint types**: unary, binary (equals, not-equals, offset, comparator, logic, element over fixed array, absolute-difference, division, predicate, tuples, subset, disjoint, intersection-cardinality), and n-ary (AllDiff, AllEqual, NotAllEqual, AtMostOne, AtLeastN, AtMostN, ExactlyOne, Sum, Product, Linear, Count, Among, Inverse, GlobalCardinality, NValue, Cumulative (fixed and variable durations/resources), Disjunctive, BinPacking, Max, Min, Element over variables, Tuples (positive, wildcard/starred, and negative/conflict), ValueDisjunction (`OR(v_i == target_i)`), ValueConjunction (`AND(v_i <op> target_i)`), Increasing, Decreasing, Lex, predicate, Circuit, Diffn, Regular)
...
- **Solver configuration**: `createSolver(csp, config)` takes an optional `SolverConfig` builder bundling search limits, cancellation, nogood-learning behavior, and restart randomization. `SolverLimits` caps work by node count and/or wall-clock time; `getSolution()` throws `LimitExceededException` (carrying `Statistics`) when a limit is hit — distinguishable from a genuine UNSAT result (`Optional.empty()`). `getSolutions()` truncates the stream silently instead. A `Cancellation` token lets a caller stop a solve from outside — e.g. from a `SolverListener` callback — mirroring the same silent-except-one-throw-site behavior. `nogoodLearningEnabled` is a tri-state `Boolean` — unset (the default) means CDCL is **off**, measured as a net cost on the bundled corpus; pass `true` to opt in or `false` to pin it off explicitly
...
- **Restart randomization**: `DomWdegLubySearch` reseeds its variable-selector tie-breaking once per Luby restart from a `RestartRandomization`, giving restart diversification a controlled, seedable source instead of an accidental one. `SolverConfig` defaults to a fresh random seed per config (so search behavior isn't reproducible run-to-run unless you pin one); pass `RestartRandomization.seeded(fixedSeed)` for reproducible randomized search or `RestartRandomization.NONE` for fully deterministic tie-breaking
...
- **XCSP3 instance parsing**: `Xcsp3Parser.parse(Path)` reads an [XCSP3](https://xcsp.org) instance file directly into a `ConstraintSatisfactionProblem` (plus an optional objective for COP instances), covering integer and symbolic (string-valued) variable domains, `intension` (a chain of shape recognizers maps common patterns — comparisons, `and`/`or`, `add`/`mul`/`dist`/`neg`/`sub`/`div`/`mod`, `in`/`notin`, `iff`/`xor`, n-ary `eq` — onto dedicated propagating constraints rather than an opaque predicate; anything else small enough is compiled into an extensional table with real GAC, and only what exceeds that is left to a general-expression evaluator), `extension`, `allDifferent` (including the multi-list "distinct vectors" form), `allEqual`, `sum`, `count`/`among`, `minimum`/`maximum`, `element`, `ordered`, `lex`, `cumulative` (fixed and variable durations/resources), `noOverlap` (1D via real edge-finding, 2D via compulsory-part propagation), `circuit`, `binPacking`, `regular`, `mdd`, `channel`, `instantiation`, `clause` (SAT), and single-variable/sum-type/maximum-type objectives. Verified against a bundled 85-instance corpus from the real XCSP3 competition archive, cross-checked with `org.xcsp.parser.callbacks.SolutionChecker` for independent solution verification
...
Coverage is a narrow MVP, not the full XCSP3-core specification: integer and symbolic (string-valued) variable domains, `intension` (including the `dist` operator, `in`/`notin` against a literal `set(...)` of constants — the `set(...)` members can themselves be variables or expressions, not just constants — and `iff`/`xor`; a two-operand `iff` where both sides independently recognize as a relation gets real reified propagation via a pair of hard-reified indicators rather than falling back to an opaque predicate), `extension` (n-ary tables: positive/support, wildcard/starred `*` entries, and negative/conflict — all with real GAC, none needing an exponential complement or expansion; the unary single-variable form supports positive and negative directly as a domain predicate), `allDifferent` (plain list, `matrix` decomposing into one `allDiffConstraint` per row plus one per column, and multi-list "distinct vectors" via `DistinctVectorsConstraint`), `allEqual`, `sum` (constant, variable, or per-term variable coefficients), `count`/`among` (constant or variable-target condition), `minimum`/`maximum` (constant or variable-target condition), `nValues`, `cardinality` (fixed-value form, fixed-occurrence, min/max-occurrence-range, and variable-occurrence-count), `element` (including an equals-to-constant condition), `ordered`, `lex` (any number of lists — beyond two, decomposes into `lists.length - 1` consecutive pairwise `LexConstraint`s), `cumulative`, `noOverlap` (1D via `disjunctiveConstraint`'s real edge-finding propagation, not just timetabling; 2D via `DiffnConstraint`/`DiffnVariableConstraint` depending on fixed vs. variable rectangle sizes; other dimensionalities throw), `circuit`, `binPacking`, `regular` (via `Automaton`/`RegularConstraint`, mapping xcsp3-tools' symbolic state names onto contiguous integer indices), `mdd` (mapped onto the same `RegularConstraint`, since its forward-backward DP already handles a layered acyclic DAG correctly with no changes — only the fixed `root`/`nodeT` start/final states differ from `regular`'s arbitrary ones), `channel` (single-array self-inverse and two-array forms via `InverseConstraint`; the single-value "hot index" form throws), `instantiation` (a fixed/partial assignment, e.g. a puzzle's "givens" — unreified decomposes into one `equalsConstraint` per pinned variable; reified routes through a single `ValueConjunctionConstraint` with real incremental propagation), `clause` (a SAT clause over literals, each citing its own variable directly via `ValueDisjunctionConstraint` rather than bridging through a boolean indicator), and single-variable, sum-type, or unweighted maximum-type `minimize`/`maximize` objectives (the last via a fresh `MaxVariableConstraint`-linked auxiliary variable; a `coeffs`-weighted maximum-type objective throws). A `group` or `slide` wrapping any of these is covered too — `xcsp3-tools` expands both back into repeated ordinary constraints (a `group` against each `args` line, a `slide` against each precomputed window) before either ever reaches jcsp's callbacks. Every constraint type above except the variable-target form of `sum` supports reification: `reifiedBy` (full, `indicator <-> body`) and `hreifiedFrom` (half, `indicator -> body`); `hreifiedTo` (`body -> indicator`) always throws, since it has no jcsp builder counterpart. `ordered` reifies via a dedicated `OrderedConstraint` (one object for the whole chain, needed since reification can't be expressed as `N-1` separate pairwise constraints); `nValues` reifies its condition comparison only, not the underlying count-to-distinct-values definition; `allDifferentMatrix`/many-list `lex` reify via a generic `AndConstraint` conjunction wrapper — each builds its underlying per-row/column or per-consecutive-pair constraints as standalone objects and wraps the set in one `AndConstraint` (a plain two-list `lex` is unaffected either way, since one `LexConstraint` already covers it). A construct outside this set — the variable-valued forms of `cardinality`'s *values* (as opposed to its occurrence counts, which are supported), other `allDifferent` variants (`Except`, symbolic), `noOverlap` with dimensionality other than 1 or 2, `channel`'s single-value form, a `minimum`/`product`/`nValues`/`lex`-type or general expression objective — fails parsing immediately with either `UnsupportedXcsp3ConstraintException` (recognized but unmappable) or the underlying `xcsp3-tools` library's own `RuntimeException` (not recognized at all), rather than silently returning an under-constrained model.

**Intension constraints are tabulated when they fit.** An `<intension>` whose variables span a small
enough space is compiled into an extensional table with real GAC, instead of being decomposed into
reified indicators and arithmetic auxiliaries. This matters most for the shapes that decompose
worst — XCSP3's knight's-move idiom is one binary predicate over two `0..35` variables, so it
tabulates to at most 1,296 tuples, where the decomposition produced two reified conjunctions plus
`div`/`mod` auxiliaries per occurrence:

| bundled instance | decomposed | tabulated |
|---|---|---|
| `KnightTour-06-int` | 180 variables, no solution in 60s | **36 variables, solved in 0.26s (303 nodes)** |
| `QueenAttacking-06` | 269 variables, no solution in 30s | **48 variables, proven optimal in 0.78s** |

Tabulation is bounded two ways, because a per-constraint limit cannot bound an instance: a 1 MiB
ceiling on any single table's support index, and a 500,000-tuple budget across the whole instance.
Past either, the decomposition is used as before — declining is always safe, so a model that does
not fit simply parses the way it always did. Candidate tuples are tested with the same expression
evaluator the general fallback uses, so tabulation changes how a constraint propagates and never
what it means. See [ADR-0034](docs/adr/0034-tabulating-small-scope-intension-constraints.md).

### Solver configuration

`createSolver` takes an optional `SolverConfig` — a builder with defaults, so new knobs don't mean new overloads. `SolverConfig.builder().build()` reproduces the library's unconfigured defaults: unlimited search, standard nogood learning, and randomized restart tie-breaking seeded fresh per config (see **Restart randomization** below — this is the one knob here that doesn't default to "off"):
...
BoundSolver solver = Solver.Factory.INSTANCE.createSolver(csp,
    SolverConfig.builder()
        .nogoodLearningEnabled(true)   // opt in; unset means off
        .build());
```

(`nogoodLearningEnabled` affects both chains uniformly.)

**Restart randomization** — `DomWdegLubySearch.getSolution()` reseeds its variable selector's tie-breaking once per Luby restart from a `RestartRandomization`, so restarts genuinely diversify (ties in the dom/wdeg ratio are broken by a fresh random draw each restart) rather than always breaking the same way. Unlike every other `SolverConfig` knob, this one is *on* by default — each `SolverConfig` gets its own fresh random base seed, so `getSolution()`'s exact output (node count, and potentially which solution comes back for a problem with multiple solutions) isn't reproducible run-to-run unless you pin a seed:

```java
BoundSolver solver = Solver.Factory.INSTANCE.createSolver(csp,
    SolverConfig.builder()
        .restartRandomization(RestartRandomization.seeded(42L))  // reproducible randomized search
        .build());
```
...
builder.linearBooleanConstraint(Map.of(b1, 2, b2, 3), Operator.LEQ, 10)     // weighted sum over Variable<Boolean> (true=coefficient, false=0)
builder.linearBooleanConstraint(Map.of(b1, 2, b2, 3), Operator.LEQ, target) // variable-target sibling
builder.countConstraint(Set.of(v1, v2, v3), value, Operator.EQ, 2)                    // number of variables equal to value == 2  (also LEQ, GEQ, etc.)
builder.countConstraint(Set.of(v1, v2, v3), value, Operator.EQ, target)               // number of variables equal to value == target  (target is a variable, not a constant)
...
builder.starredTuplesConstraint(Set.of(Map.of(v1, 1, v2, NaryStarredTuplesConstraint.STAR), ...))  // tuplesConstraint sibling with wildcard ("don't care") positions
builder.conflictTuplesConstraint(Set.of(Assignment.of(...), ...))   // tuplesConstraint's negative sibling: forbidden combinations, everything else implicitly allowed
builder.valueDisjunctionConstraint(Map.of(v1, 1, v2, 2))            // v1 == 1 OR v2 == 2  (at least one variable equals its own target value)
builder.valueConjunctionConstraint(Map.of(v1, 1, v2, 2), Operator.EQ)  // v1 == 1 AND v2 == 2  (also NEQ; every variable must meet its own target)
...
builder.notAllEqualConstraint(Set.of(v1, v2, v3))                   // not every variable equal (at least 2 distinct values); exact GAC, not just count > 1
