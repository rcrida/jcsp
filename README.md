...
- **XCSP3 instance parsing**: `Xcsp3Parser.parse(Path)` reads an [XCSP3](https://xcsp.org) instance file directly into a `ConstraintSatisfactionProblem` (plus an optional objective for COP instances), covering `intension`, `extension`, `allDifferent` (including the multi-list "distinct vectors" form), `allEqual`, `sum`, `count`/`among`, `minimum`/`maximum`, `element`, `ordered`, `lex`, `cumulative`, `circuit`, `binPacking`, `regular`, `mdd`, `channel`, `noOverlap`, `instantiation`, and single-variable/sum-type/maximum-type objectives
...
Coverage is a narrow MVP, not the full XCSP3-core specification: variable domains, `intension` (including the `dist` operator and `in`/`notin` against a literal `set(...)` of constants), `extension` (n-ary positive/support tables; the unary single-variable form supports both positive and negative — negation is a cheap predicate over one variable's own domain there, unlike the n-ary conflict-table form, whose complement could blow up combinatorially and so is rejected), `allDifferent` (plain list, `matrix` decomposing into one `allDiffConstraint` per row plus one per column, and multi-list "distinct vectors" via per-pair `AtLeastNConstraint`s over reified position-inequality indicators), `allEqual`, `sum`, `count`/`among` (constant or variable-target condition), `minimum`/`maximum` (constant or variable-target condition), `nValues`, `cardinality` (fixed-value form, both fixed-occurrence and min/max-occurrence-range), `element`, `ordered`, `lex` (any number of lists — beyond two, decomposes into `lists.length - 1` consecutive pairwise `LexConstraint`s), `cumulative`, `circuit`, `binPacking`, `regular` (via `Automaton`/`RegularConstraint`, mapping xcsp3-tools' symbolic state names onto contiguous integer indices), `mdd` (mapped onto the same `RegularConstraint`, since its forward-backward DP already handles a layered acyclic DAG correctly with no changes — only the fixed `root`/`nodeT` start/final states differ from `regular`'s arbitrary ones), `channel` (single-array self-inverse and two-array forms via `InverseConstraint`; the single-value "hot index" form throws), `noOverlap` (1D via `cumulativeConstraint(limit=1)`; 2D via `DiffnConstraint`/`DiffnVariableConstraint` depending on fixed vs. variable rectangle sizes; other dimensionalities throw), `instantiation` (a fixed/partial assignment, e.g. a puzzle's "givens" — decomposes into one `equalsConstraint` per pinned variable), and single-variable, sum-type, or unweighted maximum-type `minimize`/`maximize` objectives (the last via a fresh `MaxVariableConstraint`-linked auxiliary variable; a `coeffs`-weighted maximum-type objective throws). A `group` or `slide` wrapping any of these is covered too — `xcsp3-tools` expands both back into repeated ordinary constraints (a `group` against each `args` line, a `slide` against each precomputed window) before either ever reaches jcsp's callbacks. Every constraint type above except the variable-target form of `sum` supports reification: `reifiedBy` (full, `indicator <-> body`) and `hreifiedFrom` (half, `indicator -> body`); `hreifiedTo` (`body -> indicator`) always throws, since it has no jcsp builder counterpart. `ordered` reifies via a dedicated `OrderedConstraint` (one object for the whole chain, needed since reification can't be expressed as `N-1` separate pairwise constraints); `nValues` reifies its condition comparison only, not the underlying count-to-distinct-values definition; `allDifferentMatrix`/`allDifferentList`/`instantiation`/many-list `lex` reify via a generic `AndConstraint` conjunction wrapper — each builds its underlying per-row/column, per-pair, per-pin, or per-consecutive-pair constraints as standalone objects and wraps the set in one `AndConstraint` (a plain two-list `lex` is unaffected either way, since one `LexConstraint` already covers it). A construct outside this set — the variable-valued forms of `cardinality`, other `allDifferent` variants (`Except`, symbolic), `noOverlap` with dimensionality other than 1 or 2, `channel`'s single-value form, a `minimum`/`product`/`nValues`/`lex`-type or general expression objective — fails parsing immediately with either `UnsupportedXcsp3ConstraintException` (recognized but unmappable) or the underlying `xcsp3-tools` library's own `RuntimeException` (not recognized at all), rather than silently returning an under-constrained model.
