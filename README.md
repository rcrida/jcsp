...
- **XCSP3 instance parsing**: `Xcsp3Parser.parse(Path)` reads an [XCSP3](https://xcsp.org) instance file directly into a `ConstraintSatisfactionProblem` (plus an optional objective for COP instances), covering `intension`, `extension`, `allDifferent` (including the multi-list "distinct vectors" form), `allEqual`, `sum`, `count`/`among`, `minimum`/`maximum`, `element`, `ordered`, `lex`, `cumulative`, `circuit`, `binPacking`, `regular`, `channel`, `noOverlap`, `instantiation`, and single-variable/sum-type/maximum-type objectives
