...
/**
 * Recognizes a bare {@code eq}/{@code ne} acting as a channel between a plain variable and an
 * arbitrary compound relation -- e.g. {@code ne(and(ne(q,x),or(...)),b)}, "{@code b} is the
 * negation of whether {@code q} and {@code x} are a queen's-move apart" (XCSP3's {@code
 * QueenAttacking} idiom: {@code b[i] != attacks(q,x[i])}). Generalizes {@link IffRecognizer} two
 * ways: it fires on plain {@code eq}/{@code ne} node types, not just a literal {@code iff(...)}
 * node, and each operand is resolved via {@code dispatch} (the owning {@link
 ...
 * if that fails, treated as a bare variable -- {@link IffRecognizer}'s own {@code recognizeRelation}
 * does neither: it only tries a narrow binary/ground relation pair, and never accepts a bare-variable
 * operand at all, so it can't reach a compound {@code and}/{@code or} child or a plain channel
 * target the way this recognizer does.
 * <p>
 ...
 * <p>
 * Once both operands resolve to an indicator, {@code eq}/{@code ne} between them is exactly a
 * {@link BinaryComparatorConstraint} with {@link Operator#EQ}/{@link Operator#NEQ} -- the same
 * reduction {@link IffRecognizer} performs, just with different operand resolution and (for
 * {@code ne}) a different final operator ({@code iff} only ever needs {@code EQ}). Each side is
 * tried independently, in either order, so operand order doesn't matter: a bare-variable operand
 * never dispatches successfully on its own (every recognizer in the chain requires an
 * operator-bearing node, never a plain {@code VAR} leaf), so trying dispatch first and falling
 * back to the bare-variable case is always unambiguous.
 * <p>
 ...
 * than this class's boolean-indicator bridging), so this recognizer is only ever reached once
 * those have all declined -- i.e. when at least one side is a genuinely compound relation.
 * Declining on either side is always safe, just less propagated, falling through to the generic
 * {@link PredicateConstraint} unchanged. Confirmed via the bundled XCSP3 competition corpus
 * ({@code QueenAttacking-06.xml.lzma}, 11 occurrences).
 */
final class ChannelRecognizer implements ConstraintRecognizer {
    ...
    public Optional<Constraint> recognize(XNode<XVarInteger> tree) {
        ...
        if ((operator != Operator.EQ && operator != Operator.NEQ) || tree.sons.length != 2) {
