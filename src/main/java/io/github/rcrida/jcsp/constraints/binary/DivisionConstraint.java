...
public class DivisionConstraint<N extends Number> extends BinaryConstraint<N, N> implements Propagatable {
     ...
     * io.github.rcrida.jcsp.domains.DiscreteDomain} at all. Mirrors {@link
     * BinaryComparatorConstraint#explainInfeasible}; see its javadoc for the narrow scope of this
     * benefit (mixed discrete/bounded pairs during search). Applies uniformly to all three
     * infeasibility paths in {@link #propagate} (the two early ratio-bound checks and the inner
     * discrete-gap narrowing), since each depends only on the current dividend/divisor bounds.
     * <p>
     ...
     * constraint sharing it (see {@link BinaryOffsetConstraint#explainInfeasible}'s Javadoc for the
     * confirmed regression this pattern caused).
     */
    @Override
