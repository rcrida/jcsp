...
2. **`PropagationFixpointSolver`** — the chain's one-time preprocessing step; delegates to `FixpointPropagation` (`public`, `solver` package — needs to be, so its `PROPAGATORS`/`applyFixpoint`/etc. stay real `{@link}`-able cross-package references rather than downgrading to `{@code}`) to run every registered propagator (interval bounds clipping, AC3, AllDiff GAC, sum/linear bounds, counting/global-cardinality, cumulative timetabling, table/regular/circuit constraints, ordering, reification, and the full set-CP list) to a combined fixpoint via a static `PROPAGATORS` list; each propagator can enable others to make further reductions. `FixpointPropagation` is a free-standing algorithm, not a method on the decorator, because it's also called from `Solver.Factory.FULL_PROPAGATION_INFERENCE` (per search node) and `SetBranchingSolver` (per branch step) — neither has a `PropagationFixpointSolver` instance to call it on. Many highly-constrained problems (Zebra, Sudoku, MagicSquare) are solved entirely at this step. Adding a new propagator is a one-line `FixpointConsistency.of(MyConstraint.class)` entry. `PropagationFixpointSolver`'s own `snap` field controls `BoundedDomain` handling: `true` (satisfaction) snaps non-singleton intervals to their midpoint for one concrete solution; `false` (optimization) leaves intervals open for `BisectionConditioningSolver`. A registered `SolverListener` gets `onPropagatorProgress` notifications (propagator, before/after domain snapshots, before/after domain-sum) whenever a propagator narrows a domain, at all three of `FixpointPropagation`'s call sites; see [ADR-0010](docs/adr/0010-push-listener-for-solve-progress.md). `FixpointPropagation.applyFixpoint`/`applyFixpointWithReason` also check a registered `Cancellation` once per propagator within the round ("between propagators"), throwing `SolverCancelledException`; every caller except `DomWdegLubySearch.searchOne` catches it and converts to its own existing "stopped early" behavior. See [ADR-0011](docs/adr/0011-cancellation-token-for-main-chain-search.md).
