...
- **`Propagatable` interface** — in `consistency` package; constraints that support domain propagation implement `propagate(Map<Variable<?>, Domain<?>> domains) → Optional<Map<Variable<?>, Domain<?>>>`. `AllDiffConstraint`, `SumConstraint`, `LinearConstraint`, `CountConstraint`, `InverseConstraint`, `AmongConstraint`, and `CumulativeConstraint` implement it. `ConsistencyFixpoint` provides the shared fixpoint loop used by all consistency classes. Each constraint type has a corresponding consistency class that drives its fixpoint loop and is registered in the solver chains via `ConstraintConsistency`. `CountConstraint` and `AmongConstraint` propagation classifies variables as definite/possible/impossible, then prunes domains when the count quota is met (EQ/LEQ) or must be reached (EQ/GEQ). `InverseConstraint` propagation runs two passes of pairwise arc consistency between the `f` and `invf` arrays.
