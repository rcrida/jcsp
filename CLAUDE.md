...
- **`SetDomain<T> extends DiscreteDomain<T>`** — intermediate interface for all set-backed discrete domains; declares `values() → Set<T>` and derives every other `DiscreteDomain` method from it by default. Concrete implementors (`ObjectSetDomain`, `IntRangeDomain`, `EnumDomain`, `BooleanDomain`, `AssignmentDomain`, `NumericSetDomain`) are all records. See [ADR-0007](docs/adr/0007-record-based-domain-object-model.md) for the record-based object model and why shared behavior lives in default interface methods. `ObjectSingletonDomain` implements `DiscreteDomain` directly instead — the one implementor that isn't a `SetDomain`, since going through `values(): Set<T>` would mean materialising a throwaway single-element `Set` on every hot-path check (assigned during search, and by `SetDomain.DefaultBuilder#build` — or, for `ObjectSetDomain`/`NumericSetDomain`, their own builders' matching overrides — whenever a `SetDomain`-backed domain narrows to one remaining value). `NumericSetDomain`'s builder collapses to `NumericSingletonDomain` instead of plain `ObjectSingletonDomain`, since some of its callers (e.g. `NumericDomain#withBounds`'s default method) are typed to return `NumericDomain`, which `ObjectSingletonDomain` doesn't implement but `NumericSingletonDomain` does. `SetDomain#domainEquals` checks `instanceof DiscreteDomain` rather than `instanceof SetDomain` specifically to keep equality symmetric with it.
