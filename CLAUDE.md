...
- **`SetDomain<T> extends DiscreteDomain<T>`** — intermediate interface for all set-backed discrete domains; declares `values() → Set<T>` and derives every other `DiscreteDomain` method from it by default. Concrete implementors (`DomainObjectSet`, `IntRangeDomain`, `EnumDomain`, `BooleanDomain`, `AssignmentDomain`, `NumericDiscreteDomain`) are all records. See [ADR-0007](docs/adr/0007-record-based-domain-object-model.md) for the record-based object model and why shared behavior lives in default interface methods. `SingletonDomain` implements `DiscreteDomain` directly instead — the one implementor that isn't a `SetDomain`, since going through `values(): Set<T>` would mean materialising a throwaway single-element `Set` on every hot-path check (assigned during search, and by `SetDomain.DefaultBuilder#build` — or, for `DomainObjectSet`, its own builder's matching override — whenever a `SetDomain`-backed domain narrows to one remaining value). `NumericDiscreteDomain` is the one exception: its builder keeps Lombok's generated `build()` rather than collapsing to a `SingletonDomain`, since `NumericDomain#withBounds`'s default method (its main producer) is typed to return `NumericDomain`, which `SingletonDomain` doesn't implement. `SetDomain#domainEquals` checks `instanceof DiscreteDomain` rather than `instanceof SetDomain` specifically to keep equality symmetric with it.
