...
- **`DiscreteDomain<T> extends Domain<T>`** — enumerable domains; adds `stream()`, `toList()`, and `toBuilder()` (with inner `Builder<T>` interface). Code that needs to enumerate values should be typed to `DiscreteDomain`, not `Domain`. Its static `<T> of(T...)` factory (the generic analogue of `NumericDiscreteDomain#of`) builds via `ObjectSetDomain.builder()`, collapsing to `ObjectSingletonDomain` for a single value.
- **`SetDomain<T> extends DiscreteDomain<T>`** — intermediate interface for all set-backed discrete domains; declares `values() → Set<T>` and derives every other `DiscreteDomain` method from it by default. Concrete implementors (`ObjectSetDomain`, `IntRangeDomain`, `EnumDomain`, `BooleanDomain`, `AssignmentDomain`, `NumericSetDomain`) are all records. See [ADR-0007](docs/adr/0007-record-based-domain-object-model.md) for the record-based object model and why shared behavior lives in default interface methods. `ObjectSingletonDomain`/`ObjectEmptyDomain` implement `DiscreteDomain` directly instead — the two implementors that aren't `SetDomain`s, since going through `values(): Set<T>` would mean materialising a throwaway `Set` on every hot-path check (assigned during search, and by `SetDomain.DefaultBuilder#build` — or, for `ObjectSetDomain`/`NumericSetDomain`, their own builders' matching overrides — whenever a `SetDomain`-backed domain narrows to one or zero remaining values). `NumericSetDomain`'s builder collapses to `NumericSingletonDomain`/`NumericEmptyDomain` instead of the plain `Object*` versions, since some of its callers (e.g. `NumericDomain#withBounds`'s default method) are typed to return `NumericDomain`, which the `Object*` versions don't implement but the `Numeric*` versions do. `ObjectEmptyDomain`/`NumericEmptyDomain` are plain final classes with a private constructor and a single shared instance, not records like every other domain type — a `public record`'s canonical constructor can't be more restrictive than the record itself (JLS 8.10.4.2), so a record couldn't enforce the one-instance property they exist for (added for symmetry with the singleton types, not because empty-domain construction is a measured hot path — an empty domain is checked once with `isEmpty()` and discarded, never read repeatedly). `SetDomain#domainEquals` checks `instanceof DiscreteDomain` rather than `instanceof SetDomain` specifically to keep equality symmetric with all of these.
